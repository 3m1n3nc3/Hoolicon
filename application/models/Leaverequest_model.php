...
class Leaverequest_model extends CI_model {
    ...
    public function allotedLeaveType($id) {
        ...
        $query = $this->db->select('staff_leave_details.*,leave_types.type,leave_types.id as typeid')->where(array('staff_id' => $id))->join("leave_types", "staff_leave_details.leave_type_id = leave_types.id")->group_by("leave_types.id, staff_leave_details.id")->get("staff_leave_details");
