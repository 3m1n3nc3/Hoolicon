...
class Paymentsettings extends Admin_Controller {
    ...
    public function remita() {
        ...
        $this->form_validation->set_rules('remita_public_key', 'Public Key', 'trim|required|xss_clean');
        $this->form_validation->set_rules('remita_secret', 'Secret', 'trim|required|xss_clean');
        if ($this->form_validation->run()) {
            $data = array(
                'api_publishable_key' => $this->input->post('remita_public_key'),
                'api_secret_key' => $this->input->post('remita_secret'),
                'payment_type' => 'remita',
            );
            $this->paymentsetting_model->add($data);
            echo json_encode(array('st' => 0, 'msg' => "Record updated successfully"));
            ...
            $data = array(
                'remita_public_key' => form_error('remita_public_key'),
                'remita_secret' => form_error('remita_secret')
            );
            echo json_encode(array('st' => 1, 'msg' => $data));
        }
    }
    ...
    public function paystack() {
        ...
        $this->form_validation->set_rules('paystack_public_key', 'Public Key', 'trim|required|xss_clean');
        $this->form_validation->set_rules('paystack_secret', 'Secret', 'trim|required|xss_clean');
        if ($this->form_validation->run()) {
            $data = array(
                'api_publishable_key' => $this->input->post('paystack_public_key'),
                'api_secret_key' => $this->input->post('paystack_secret'),
            ...
            $data = array(
                'paystack_public_key' => form_error('paystack_public_key'),
                'paystack_secret' => form_error('paystack_secret')
            );
            echo json_encode(array('st' => 1, 'msg' => $data));
        }
    }
