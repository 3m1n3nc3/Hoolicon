...
class Parents extends Parent_Controller {
    ...
    function dashboard() {
        ...
        $data['child_id_0'] = $this->children[0]['student_id'];
    ...
    function edit($id) {
       ...
       // $data["student_categorize"] = $student_categorize;
        ...
        if ($this->form_validation->run() == FALSE) { 
            $this->load->view('layout/parent/header', $data);
            $this->load->view('parent/parentEditOwn', $data);
            $this->load->view('layout/parent/footer', $data);
        } else {
            foreach ($this->children as $ch) { 
                $img_sid = $id;
                $student_id = $id = $ch['student_id'];
                ...
                if (isset($_FILES["father_pic"]) && !empty($_FILES['father_pic']['name'])) {
                    ...
                    $img_name = $this->fmt_sch_name . $img_sid . "father" . '.' . $fileInfo['extension'];
                ...
                if (isset($_FILES["mother_pic"]) && !empty($_FILES['mother_pic']['name'])) { 
                    ...
                    $img_name = $this->fmt_sch_name . $img_sid . "mother" . '.' . $fileInfo['extension'];
                ...
                if (isset($_FILES["guardian_pic"]) && !empty($_FILES['guardian_pic']['name'])) { 
                    ...
                    $img_name = $this->fmt_sch_name . $img_sid . "guardian" . '.' . $fileInfo['extension'];
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Parent Record Updated successfully</div>');
            redirect('parent/parents/edit/'.$id);
        }
    }
