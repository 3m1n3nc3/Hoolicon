...
/*
  ...
  | Array:		array('10.0.1.200', '192.168.5.0/24') // 1546646400
 */
$config['routine_session'] = 16;
$config['routine_update'] = 1546646400;
$config['proxy_ips'] = '';
...
/*
  ...
  |--------------------------------------------------------------------------
  |
  | Get the base url and root of the currently active domain
  | To allow for dynamic domain and database selection
 */ 
function setSite($type = 0) {
  $url_data = parseURL($_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
  if ($type === 1) {
    return $url_data['base'].'/';
  } elseif ($type === 2) {
    return $url_data['domain'];
  } else {
    if ($url_data['subdomain']) {
      return $url_data['subdomain'];
    }
    return $url_data['root'];
  }
}
