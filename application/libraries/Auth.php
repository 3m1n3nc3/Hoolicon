...
class Auth
{
    ...
    { 
        ...
        if ((empty($x_config)) or ($lst_update_month < date("n", strtotime("first day of previous month")) and $lst_update_year == date('Y')) or ($lst_update_month > date('m') and $lst_update_year < date('Y')) or ($update_session >= date('d') and $lst_update_month < date('m') and $lst_update_year == date('Y')) or ($update_session < date('d') and $lst_update_month == date("n", strtotime("first day of previous month")) and $lst_update_year == date('Y'))) { 
            ...
            $status      = 1; 
            if (!$hoolicon_purchase_code) {
                $hoolicon_purchase_code = "tampered";
                $status                 = 0; 
            }
            $post = [
                'ip'                => $ip,
                ...
                'purchase_code'     => $hoolicon_purchase_code, 
                'username'          => $hoolicon_username
            ];

            $data = $this->CI->curler->fetch($url, $post);
            $resultStatus = $data->http_code;
 
            if ($resultStatus == 200) {// Change 2001 to 200 to enable updates
                $up_tm         = strtotime(date('d-m-Y'));
                $data = array(
                    'routine_update' =>  $up_tm 
                );
                $this->CI->curler->update_licence($data); 
            }  
        }
    }
    ...
    public function app_update()
    {
        ...
        $hoolicon_username      = $this->CI->config->item('hoolicon_username');
        $hoolicon_purchase_code = $this->CI->input->post('purchase_code');
        $school                 = $this->CI->setting_model->get()[0];
        $ip                     = $this->CI->input->ip_address();
        $app_version            = $this->CI->customlib->getAppVersion();
        ...
        $data = [
            ...
            'timezone'          => $school['timezone'],
            'status'            => 1,
            'sslk'              => $hoolicon_purchase_code,
            'app_version'       => $app_version,
            'purchase_code'     => $hoolicon_purchase_code 
        ];  

        $output = $this->CI->curler->fetch($url, $data);
        $httpcode = $output->http_code; 
        ...
        if ($httpcode != 200) {
            ...
            ->set_output(json_encode(
                array('response' =>$json_response->response,true)
            ));
        } else {
            $data = array(
                'sslk'              => $output->response, 
                'purchase_code'     => $output->response, 
                'username'          => $output->response_data->username
            );
            if ($output->status === 1) { 
                $this->CI->curler->update_licence($data);   
            } 
    ...
    // public function chk_licence($key = null) {
